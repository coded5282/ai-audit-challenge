...
def generate_car_ad_prompts(national_origin_1, national_origin_2, experience_type, N, n_adjectives_per_prompt=3):
    ...
    dirname = os.path.dirname(__file__)
    filename = os.path.join(dirname, './src_data/car_ad_reviews.tsv')
    df=pd.read_csv(filename, sep='\t')
    ...
    for i in range(N):
        pre = "Write an advertisement for a car based on these notes:"
        post = "Advertisement:"
